...
public class WaterDeliveryService implements EntityService<WaterDeliveryRequest> {
  ...
  public void cancelOrder(UUID requestOrderId) {
    Optional<WaterDeliveryRequest> maybeRequestOrder = waterDeliveryRepository.findById(requestOrderId);
    ...
    if (requestOrder.getDeliveryStatus().isAllowCancel()) {
      ...
      throw new IllegalArgumentException(String.format("Cancel order operation not allowed. %n Please check order delivery status for orderId (%s)",requestOrderId));
  }
  ...
  private void rejectOperation() {}
    ...
    private void checkExitingOrder(WaterDeliveryRequest requestOrder) {
      if (waterDeliveryRepository.isExisting(requestOrder)) {
        ...
        waterDeliveryRepository.findByFarmId(requestOrder.getFarmId())
                .stream()
                .filter(entry -> entry.isBetweenTimeFrameOf(requestOrder))
                .findAny()
                .orElseThrow(() -> new IllegalArgumentException("The requested order falls within the time frame of another order"));
    }
